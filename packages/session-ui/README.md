...
- `styles.ts` maps Jouzu semantic roles to the default Session UI colors; `color.ts` maps custom RGB roles to truecolor, 256-color, 16-color, or plain terminal output. Renderers contain no raw color choices or legacy style names, and a future global theme can replace the complete mapping.
