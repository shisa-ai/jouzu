...
When the isolated Jouzu agent root has no `keybindings.json`, the first interactive launch seeds `Tab` for Pi's `app.message.followUp` action and `Ctrl+Up` for `app.message.dequeue`. An open autocomplete menu keeps Tab and selector navigation before app actions. Existing files are never changed automatically. Use `jz keybindings status|plan|apply|reset`; explicit apply merges only missing defaults with conflict checks/backups, while reset removes only Jouzu-recorded entries and disables reseeding. `JOUZU_NO_KEYBINDING_DEFAULTS=1` is a one-run opt-out. Modified-arrow reporting is terminal-dependent; tmux should use `extended-keys-format csi-u`.
