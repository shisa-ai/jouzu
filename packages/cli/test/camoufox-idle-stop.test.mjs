...
test("open preserves an in-flight call until it settles", () => {
	const scheduler = makeScheduler();
	const stops = [];
	const idleStop = createCamoufoxIdleStop({ timeoutMs: 5_000, scheduler, stop: () => stops.push(1) });
	idleStop.onCallStart();
...
test("repeated open preserves the armed timer and in-flight calls", () => {
	const scheduler = makeScheduler();
	let stops = 0;
	const idleStop = createCamoufoxIdleStop({ timeoutMs: 5_000, scheduler, stop: () => stops++ });
	idleStop.onCallSettled();
	idleStop.open();
	assert.equal(scheduler.armed().length, 1, "a repeated session start must not discard the idle timer");
	idleStop.onCallStart();
	idleStop.open();
	scheduler.timers[0].fire();
	assert.equal(stops, 0, "a repeated session start must not forget an active call");
	idleStop.onCallSettled();
	scheduler.timers.at(-1).fire();
	assert.equal(stops, 1);
});

test("a call surviving close and open cannot make a newer call appear idle", () => {
	const scheduler = makeScheduler();
	let stops = 0;
	const idleStop = createCamoufoxIdleStop({ timeoutMs: 5_000, scheduler, stop: () => stops++ });
	idleStop.onCallStart();
	...
	idleStop.onCallSettled();
	scheduler.timers.at(-1).fire();
	assert.equal(stops, 0, "the newer call is still in flight after the older call settles");
	idleStop.onCallSettled();
	scheduler.timers.at(-1).fire();
	assert.equal(stops, 1);
});
...
test("a wedged idle-stop close does not block session_shutdown past the grace period", async () => {
	...
	try {
		...
		handlers.get("session_start")({}, { cwd: stateDir });
		let secondShutdownSettled = false;
		const secondShutdown = handlers
			.get("session_shutdown")()
			.then(() => {
				secondShutdownSettled = true;
			});
		// Flush microtasks without advancing the grace timer. No browser was used
		// in this session, so its shutdown must not await the previous close.
		await new Promise((resolve) => setImmediate(resolve));
		assert.equal(secondShutdownSettled, true, "an expired drain must not delay another session");
