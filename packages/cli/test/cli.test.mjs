...
test("a non-interactive first run uses Core without recording Japanese consent", () => {
	...
	try {
		...
		assert.equal(state.profileVersion, 11);
...
test("self-update avoids loading Pi runtime modules before package replacement", () => {
	const temp = mkdtempSync(join(tmpdir(), "jouzu-self-update-imports-"));
	try {
		const marker = join(temp, "pi-loaded.txt");
		const preload = join(temp, "track-pi.mjs");
		const loader = join(temp, "track-pi-loader.mjs");
		writeFileSync(
			preload,
			`import { register } from "node:module"; register(new URL("./track-pi-loader.mjs", import.meta.url));`,
		);
		writeFileSync(
			loader,
			`import { writeFileSync } from "node:fs";
export async function load(url, context, nextLoad) {
  if (url.includes("/@earendil-works/pi-coding-agent/")) writeFileSync(process.env.JOUZU_IMPORT_MARKER, url);
...
`,
		...
		assert.equal(status.status, 0, status.stderr);
		assert.equal(existsSync(marker), false, "self-update loaded Pi's runtime");

		const version = run(["--version"], { env });
		assert.equal(version.status, 0, version.stderr);
		assert.equal(existsSync(marker), true, "the import detector did not observe normal Pi loading");
	} finally {
		rmSync(temp, { recursive: true, force: true });
	}
});
...
test("shows Jouzu help and leaves Pi help behind the explicit escape", () => {
	...
	assert.match(jouzuHelp.stdout, /Open the Jouzu Palette/);
	assert.match(jouzuHelp.stdout, /Ctrl\+L/);
	assert.match(jouzuHelp.stdout, /Tab\nor Shift\+Tab switches Palette sections/u);
