...
test("trace bounds serialized previews containing JSON escapes", async () => {
	await fixture(
		Array.from({ length: 10 }, (_, i) => message(String(i), { role: "user", content: '"\\\n'.repeat(30000) })),
		async (path) => {
			const page = await readSessionTrace(path, { limit: 100 });
			assert.ok(Buffer.byteLength(JSON.stringify(page)) <= 48000);
			for (const record of page.records) assert.ok(Buffer.byteLength(JSON.stringify(record)) <= 24000);
