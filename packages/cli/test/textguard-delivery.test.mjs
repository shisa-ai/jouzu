...
test(
	"registered review command releases checked skill bytes through reload to the provider",
	{ timeout: 15000 },
	async () => {
		...
		try {
			...
			assert.equal(
				dialogs.filter((dialog) => dialog.options.includes("Allow this content for this session")).length,
				2,
			);
		} finally {
			session?.dispose();
