...
export function buildModelGuidance(modelId: string | undefined, selectedTools: readonly string[] = []): string {
	...
	const guidance = [
		...
		"When the user points you at a problem, assume they expect you to diagnose and fix the issues you identify within the requested scope, then verify the fixes. Do not stop at findings or ask whether to implement an authorized fix. If the user explicitly requests diagnosis only, review only, or otherwise limits changes, respect that boundary.",
