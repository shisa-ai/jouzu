...
export class PiSessionFlowIngress implements Ingress {
	...
	private async startScheduling(resumed = false): Promise<void> {
		...
		const health = new FlowWaitHealthMonitor({
			...
			probe: (handle: FlowWaitHandle, signal: AbortSignal) =>
				branch.attachment.waitProducers.probeExecution(handle.producer, handle.execution, signal),
	...
	archiveSubmissionHistory() {
		return this.manage((service) => service.archiveSubmissionHistory());
	}
	...
	retireResultHistory(keepManifests?: number, keepBranches?: number) {
		return this.manage((service) => service.retireResultHistory(keepManifests, keepBranches));
	}
	retireWaitHistory(includeUserWork = false) {
		return this.manage((service) => service.retireWaitHistory(includeUserWork));
	}

	changeWorkStatus(id: string, status: FlowWorkStatus, reason: string) {
		return this.manage((service) =>
			service.changeWorkStatus(id, status, reason, this.options.autoRelease?.clock?.now() ?? Date.now()),
		);
	}
