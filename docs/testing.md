...
`flow-subagent-waits.test.mjs` checks authority capture, terminal races, restored subscriptions, and session identity. `flow-subagent-wake.test.mjs` exercises the assembled runtime's completion, failure, cancellation, and deadline wake-ups without polling.
