...
`flow-task-adapter.test.mjs` loads the installed pi-tasks extension and the production flow assembly. It tests background execution and waits from a task continuation, waiting on a job started by the task's parent user turn (including health monitoring), explicit user-input and pause holds, dependency ordering, three admitted attempts without progress, task ownership across resume, and rejection of an unadapted extension borrowing a previous user turn. `flow-task-producer.test.mjs` covers stale revisions, completion, deletion, foreign identities, and preservation of a separate flow pause. `flow-task-patch.test.mjs` checks the pinned build patch and refusal to overwrite unexpected package bytes. The consumed-queue race test completes a task before provider dispatch and checks that no stale request reaches the provider. `flow-task-context-guard.test.mjs` checks cancellation notes for mixed input, abort failures, and persisted cancellation markers. Mixed input retains its source bytes; ignoring the cancelled task instruction depends on the model following the added note. These tests run in the default suite without another checkout.

For subagents, `subagent` launch/resume returns `waitDependency` in its result when flow control is on. Copy that object into `agent_wait.on` and omit `work` to wait as the current invocation. The `subagent` producer's `terminal` predicate names one run: completion satisfies it; failure, interruption, or cancellation ends the wait with a failure decision. Resume creates a separate run identity. These waits have a deadline but no health policy, so omit `health` and `checkAfter`. Deadline expiry wakes the agent without stopping the child. Read and verify the child output after waking.

`flow-subagent-waits.test.mjs` checks authority capture, terminal races, restored subscriptions, and session identity. `flow-subagent-wake.test.mjs` exercises the assembled runtime's completion, failure, cancellation, and deadline wake-ups without polling. Subagent and schedule waits require a receipt registered when the producer starts the work; a caller cannot assign an existing run to another work by constructing a handle.

`schedule_prompt` with `action: "add"` returns a `waitDependency` when flow control is on. Its `schedule` producer and `first-trigger` predicate wait for the newly created schedule to start its first attempt. This applies to one-shot and recurring schedules, with or without a model. It does **not** mean the scheduled prompt has finished or succeeded. Copy the receipt into `agent_wait.on`, omit `work`, `health`, and `checkAfter`, and supply a deadline. Removing or disabling the schedule before its first trigger ends the wait with a failure decision; deadline expiry wakes the agent without changing the schedule. Schedule edits retain the same first-trigger identity. Use the receipt only for the first trigger, not a later recurrence.
