...
Cache warming replays a prompt with a one-token output limit to refresh a provider's cached prompt before it expires. These requests can incur token charges. They do not add assistant turns or tool calls to the conversation.
...
Refreshes request at most one output token and disable provider retries. The main session checks that the replay matches the admitted request before sending it. Changed or unsupported payloads are refused without failing the conversational turn. Refresh failures are best-effort and can be tried again at a later scheduled interval.
