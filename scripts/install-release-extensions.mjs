...
if (sourceOnly) {
	...
	// This mode leaves a different layout than the guarded install, so a receipt written
	// by a normal build must not outlive it and make the next build skip its install.
	rmSync(releaseBundleReceiptPath(cli), { force: true });
}

async function installReleaseBundle() {
...
// The install and the rewrites above cost about 13s per build and are a pure function of
// the fingerprinted inputs. Skip them when the tree already matches, which keeps a
// routine build from reinstalling 514 packages. A root `npm ci` deletes this tree and
// reinstalls a different layout, which removes the receipt and its recorded entries.
// Source-only builds use junctions into the root install and stay unguarded.
const fingerprint = sourceOnly ? undefined : releaseBundleFingerprint(root);
if (fingerprint !== undefined && releaseBundleIsCurrent(cli, fingerprint)) {
	console.log("release-owned extension bundle is current; skipping the install and bundle steps");
} else {
	...
	if (fingerprint !== undefined) {
		try {
			...
			console.warn(`could not record the release-bundle receipt; the next build will reinstall: ${reason}`);
		}
	}
}
