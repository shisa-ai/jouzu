...
Jouzu includes a built-in `shisa-api` source. When the `SHISA_API_KEY` environment variable holds a bearer token, that source reads the account's model catalog from `https://api.shisa.ai/v1/jouzu/model-catalog` without any configuration. On interactive startup Jouzu refreshes each enabled source whose credential is available, in the background, while cached catalogs keep serving; sources with a missing credential are not contacted and report no error. The token authenticates catalog and inference requests to the configured gateway origin and is never written to configuration, cache, or diagnostics.
