...
Jouzu also owns two semantic actions with code defaults, rebindable through the same `keybindings.json`:

| Key | Jouzu action | Behavior |
| --- | --- | --- |
| `Space`, `Ctrl+Shift+S` | `jouzu.model.toggleFavorite` | Toggle the selected model's favorite status; `Space` types while the search field holds focus |
| `Shift+Enter` | `jouzu.model.selectProjectDefault` | Select the model and store it as the project default |

`Ctrl+Shift+S` needs an enhanced-keyboard protocol; Kitty maps it to paste-from-selection by default, so Kitty users unmap it or rebind the action. `Space` remains available in every terminal.
