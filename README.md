...
| --- | --- | --- |
| `Shift+Enter`, `Ctrl+Shift+S` | `jouzu.model.toggleFavorite` | Toggle the selected model's favorite status; both work while the search field holds focus |

`Shift+Enter` and `Ctrl+Shift+S` need an enhanced-keyboard protocol; Kitty maps `Ctrl+Shift+S` to paste-from-selection by default, so Kitty users unmap it or rebind the action.
