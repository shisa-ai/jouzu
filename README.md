...
Download the unsigned Windows x64 installer from the [latest release](https://github.com/shisa-ai/jouzu/releases/latest). It bundles Node.js/npm, Git Bash, Windows Terminal, and native tools, with desktop and Start menu shortcuts. The launcher shows the working folder before it opens a terminal and can remember it for later launches; if the saved folder is unavailable, it asks for another folder instead of opening a different one. Windows may show an unknown-publisher warning. The preview has been tested on Windows Server 2025; clean Windows 10/11 testing remains pending. See [Windows installer preview](https://github.com/shisa-ai/jouzu/blob/main/packaging/windows/README.md) for setup and testing details.
...
Settings includes a **Shisa AI** row above the catalog list. Signed out, it reports `Not connected` and the signup credits, and `Enter` starts browser approval in the panel; `Esc` cancels sign-in without closing Settings. A saved sign-in shows its organization and dashboard address; an environment key shows `Connected · SHISA_API_KEY` without a saved account's organization. `D` signs out after confirmation. The row scrolls away as you move down the catalog list and returns when you navigate back to the top. After sign-in, Jouzu refreshes catalogs and model availability without restarting. If server confirmation fails or is interrupted after credentials are saved, the panel keeps the warning and recovery instructions. Signup offers are not an account balance.
