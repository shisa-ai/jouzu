...
| --- | --- | --- |
| `Ctrl+Shift+S` | `jouzu.model.toggleFavorite` | Toggle the selected model's favorite status while browsing or searching |

`Ctrl+Shift+S` needs an enhanced-keyboard protocol; Kitty maps it to paste-from-selection by default, so Kitty users unmap it or rebind the action.
