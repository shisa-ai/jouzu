...
`Ctrl+F` encodes as a plain control byte, so it works without an enhanced keyboard protocol. Inside the Models search field it replaces Pi's emacs-style cursor-right; `→` remains available there.
